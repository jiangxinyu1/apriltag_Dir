...
void print_timing(const std::string& label,
...
void preprocess_image(const cv::Mat& orig,
                      cv::Mat& filtered,
                      cv::Mat& filteredSeg,
...
void extract_edges(const cv::Mat& filteredSeg,
                   const cv::Mat& filteredMag,
...
void fit_segments(const cv::Mat& filteredSeg,
                  const cv::Mat& filteredMag,
                  const cv::Mat& filteredTheta,
                  UnionFindSimple& uf,
...
std::vector<TagDetection> decode_quads(const cv::Mat& filtered, const std::vector<Quad>& quads, const TagFamily& family)
{
    //================================================================
    ...
    for (std::vector<TagDetection>::const_iterator it = detections.begin(); it != detections.end(); it++)
    {
