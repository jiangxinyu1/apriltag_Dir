    ...
    auto checkTagPose = [&]( bool optimized )
    {
            if ( id3ready && id6ready )
            {
                ...
                if ( 0 )
                {
                    ...
                    cv::Rect rect1(int(point_uv_1[0]/point_uv_1[2]-5), int(point_uv_1[1]/point_uv_1[2]-5), 10, 10);//左上坐标（x,y）和矩形的长(x)宽(y)
                ...
                if ( 0 )
                {
                    image_check.at<uint8_t>(std::round(point_uv_2[1]/point_uv_2[2]),std::round(point_uv_2[0]/point_uv_2[2])) = 255;
                     cv::Rect rect(int(point_uv_2[0]/point_uv_2[2]-5), int(point_uv_2[1]/point_uv_2[2]-5), 10, 10);//左上坐标（x,y）和矩形的长(x)宽(y)
